...
class _AlumnoBuscarInstitucionesPageState
  ...
  static const List<String> _paises = [
    'Argentina', 'Bolivia', 'Brasil', 'Chile', 'Colombia', 'España',
        ...
        child: StatefulBuilder(
          builder: (ctx, setSheetState) => Padding(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  const Expanded(child: Text('Módulo extracurricular', style: TextStyle(fontWeight: FontWeight.w800))),
                  TextButton(onPressed: () => setSheetState(tmp.clear), child: const Text('Limpiar')),
                ]),
                ...
                for (final bloque in BloqueExtracurricularX.ordered())
                  CheckboxListTile(
                    ...
                    onChanged: (value) => setSheetState(() {
                      if (value == true) tmp.add(bloque); else tmp.remove(bloque);
                    }),
                  ),
                const SizedBox(height: 8),
  ...
  Future<void> _buscar() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _resultados = results; _cargando = false; });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _cargando = false;
  ...
  String _levelLabel(NivelCurricular value) {
    switch (value) {
      case NivelCurricular.jardin: return 'Jardín';
      case NivelCurricular.primaria: return 'Primaria';
      case NivelCurricular.secundaria: return 'Secundaria';
      case NivelCurricular.tecnica: return 'Técnica';
      case NivelCurricular.terciario: return 'Terciario';
    }
  }

  String _turnoLabel(TurnoCurricular value) {
    switch (value) {
      case TurnoCurricular.manana: return 'Mañana';
      case TurnoCurricular.tarde: return 'Tarde';
      case TurnoCurricular.noche: return 'Noche';
    }
  }

  String _tipoLabel(TipoInstitucion value) {
    switch (value) {
      case TipoInstitucion.jardin: return 'Jardín';
      case TipoInstitucion.primaria: return 'Primaria';
      case TipoInstitucion.secundaria: return 'Secundaria';
      case TipoInstitucion.tecnica: return 'Técnica';
      case TipoInstitucion.terciario: return 'Terciario';
      case TipoInstitucion.taller: return 'Taller';
      case TipoInstitucion.club: return 'Club';
      case TipoInstitucion.otra: return 'Otra';
    }
  }

  String _modalidadLabel(ModalidadCursado value) {
    switch (value) {
      case ModalidadCursado.presencial: return 'Presencial';
      case ModalidadCursado.remoto: return 'Remoto';
      case ModalidadCursado.hibrido: return 'Híbrido';
    }
  }

  String _distanciaLabel() => _distanciaMaxima == null
      ? 'Sin límite de distancia'
      : 'Hasta ${_distanciaMaxima!.toStringAsFixed(0)} km';
  ...
  Widget _scopeCard({required AlumnoBusquedaScope scope, required IconData icon}) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(color: selected ? cs.primary : cs.outlineVariant, width: selected ? 1.5 : 1),
      ),
      child: InkWell(
        borderRadius: BorderRadius.circular(16),
        ...
        child: Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(
                scope == AlumnoBusquedaScope.curricular ? 'Curricular' : 'Extracurricular',
                style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900),
              ),
              const SizedBox(height: 6),
              Text(_scopeDescription(scope)),
            ])),
            Radio<AlumnoBusquedaScope>(value: scope, groupValue: _scope, onChanged: (_) => _seleccionarScope(scope)),
          ]),
        ),
      ),
    );
  ...
  Widget _buildFilters() {
    ...
    return Column(children: [
      ...
      TextField(controller: _nombreCtrl, decoration: inputDecoration('Nombre de la institución', Icons.search)),
      ...
      DropdownButtonFormField<String>(
        ...
        items: [
          const DropdownMenuItem<String>(value: null, child: Text('Todos los países')),
          ..._paises.map((pais) => DropdownMenuItem<String>(value: pais, child: Text(pais))),
        ],
        onChanged: (value) => setState(() { _pais = value; _provincia = null; _regionCtrl.clear(); }),
      ),
      const SizedBox(height: 10),
      Wrap(spacing: 8, runSpacing: 8, children: [
        OutlinedButton.icon(
          onPressed: _cargandoUbicacion ? null : _usarMiUbicacion,
          icon: _cargandoUbicacion ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.my_location),
          label: Text(_usarUbicacion ? 'Ubicación actual activada' : 'Usar mi ubicación'),
        ),
        if (_usarUbicacion)
          OutlinedButton.icon(onPressed: _quitarUbicacion, icon: const Icon(Icons.close), label: const Text('Quitar ubicación')),
      ]),
      if (_usarUbicacion) ...[
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: OutlinedButton.icon(onPressed: _mostrarFiltrosUbicacion, icon: const Icon(Icons.social_distance), label: Text(_distanciaLabel()))),
          const SizedBox(width: 8),
          Expanded(child: SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, value: _ordenarPorDistancia, title: const Text('Más cercanas primero'), onChanged: (value) => setState(() => _ordenarPorDistancia = value))),
        ]),
      ],
      const SizedBox(height: 10),
      if (_pais == null || _pais == 'Argentina')
        DropdownButtonFormField<String>(
          ...
          items: [
            ...
            ..._provinciasArgentina.map((p) => DropdownMenuItem<String>(value: p, child: Text(p))),
          ],
          onChanged: (value) => setState(() => _provincia = value),
        )
      else
        TextField(controller: _regionCtrl, decoration: inputDecoration('Provincia, estado o región', Icons.map_outlined)),
      const SizedBox(height: 10),
      ...
      if (scope == AlumnoBusquedaScope.curricular) ...[
        DropdownButtonFormField<NivelCurricular>(
          ...
          items: [
            const DropdownMenuItem<NivelCurricular>(value: null, child: Text('Todos los niveles')),
            ...NivelCurricular.values.map((e) => DropdownMenuItem(value: e, child: Text(_levelLabel(e)))),
          ],
        ...
        DropdownButtonFormField<TurnoCurricular>(
          ...
          items: [
            const DropdownMenuItem<TurnoCurricular>(value: null, child: Text('Todos los turnos')),
            ...TurnoCurricular.values.map((e) => DropdownMenuItem(value: e, child: Text(_turnoLabel(e)))),
          ],
          onChanged: (value) => setState(() => _turno = value),
        ),
        const SizedBox(height: 10),
        const InputDecorator(
          decoration: InputDecoration(labelText: 'Ciclo curricular', border: OutlineInputBorder()),
          child: Text('El ciclo curricular todavía no está almacenado como dato canónico independiente en el perfil institucional.'),
        ),
      ] else ...[
        InkWell(
          ...
          child: InputDecorator(
            ...
            child: Text(_bloques.isEmpty ? 'Todos los módulos' : _bloques.map((e) => e.label).join(' • '), maxLines: 2, overflow: TextOverflow.ellipsis),
          ),
        ),
        const SizedBox(height: 10),
        TextField(controller: _edadCtrl, keyboardType: TextInputType.number, decoration: inputDecoration('Edad', Icons.cake_outlined).copyWith(hintText: 'Ejemplo: 12')),
        const SizedBox(height: 10),
        DropdownButtonFormField<AlumnoPrecioFiltro>(
          value: _precio,
          decoration: inputDecoration('Valor', Icons.payments_outlined),
          items: AlumnoPrecioFiltro.values.map((e) => DropdownMenuItem(value: e, child: Text(e == AlumnoPrecioFiltro.todos ? 'Todos los valores' : e == AlumnoPrecioFiltro.gratuitos ? 'Gratuitos' : 'Con costo'))).toList(),
          onChanged: (value) => setState(() => _precio = value ?? AlumnoPrecioFiltro.todos),
        ),
      ],
      const SizedBox(height: 10),
      DropdownButtonFormField<TipoInstitucion>(
        ...
        items: [
          ...
          ...TipoInstitucion.values.map((e) => DropdownMenuItem(value: e, child: Text(_tipoLabel(e)))),
        ],
      ...
      DropdownButtonFormField<ModalidadCursado>(
        ...
        items: [
          const DropdownMenuItem<ModalidadCursado>(value: null, child: Text('Todas las modalidades')),
          ...ModalidadCursado.values.map((e) => DropdownMenuItem(value: e, child: Text(_modalidadLabel(e)))),
        ],
      ...
      const SizedBox(height: 8),
      Text('Los filtros se aplican sobre los datos actualmente registrados en los perfiles de Atena.', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: cs.onSurfaceVariant)),
    ]);
  }
  ...
  Widget _buildResultCard(AlumnoInstitucionSearchResult result) {
    ...
    final niveles = inst.planConfig?.niveles.where((e) => e.habilitado).map((e) => _levelLabel(e.nivel)).toList() ?? const <String>[];
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(inst.nombre, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900))),
            IconButton(tooltip: 'Ver perfil', onPressed: () => _mostrarInstitucion(inst), icon: const Icon(Icons.account_balance_outlined)),
          ]),
          if (location.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 3), child: Text(location)),
          if (result.distanciaKm != null) Padding(padding: const EdgeInsets.only(top: 3), child: Text('${result.distanciaKm!.toStringAsFixed(1)} km de distancia')),
          const SizedBox(height: 10),
          Wrap(spacing: 6, runSpacing: 6, children: [
            if (inst.curricular) const Chip(label: Text('Curricular')),
            if (inst.extracurricular) const Chip(label: Text('Extracurricular')),
            Chip(label: Text(_tipoLabel(inst.tipoInstitucion))),
            Chip(label: Text(_modalidadLabel(inst.modalidad))),
          ]),
          if (niveles.isNotEmpty) ...[const SizedBox(height: 8), Text('Niveles: ${niveles.join(' • ')}')],
          if (bloques.isNotEmpty) ...[const SizedBox(height: 8), Text('Módulos: ${bloques.map((e) => e.label).join(' • ')}')],
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: () => _mostrarInstitucion(inst), icon: const Icon(Icons.account_balance_outlined), label: const Text('Ver perfil'))),
            const SizedBox(width: 8),
            Expanded(child: FilledButton.icon(onPressed: () => _solicitarVacante(inst), icon: const Icon(Icons.how_to_reg_outlined), label: const Text('Solicitar vacante'))),
          ]),
        ]),
      ),
    );
      ...
      final choice = await showModalBottomSheet<AlumnoBusquedaScope>(
        ...
        builder: (ctx) => SafeArea(child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Text('Solicitar vacante', style: Theme.of(ctx).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
            const SizedBox(height: 6),
            const Text('Elegí qué tipo de propuesta querés explorar.'),
            const SizedBox(height: 14),
            FilledButton.icon(onPressed: () => Navigator.pop(ctx, AlumnoBusquedaScope.curricular), icon: const Icon(Icons.school_outlined), label: const Text('Curricular')),
            const SizedBox(height: 8),
            FilledButton.icon(onPressed: () => Navigator.pop(ctx, AlumnoBusquedaScope.extracurricular), icon: const Icon(Icons.category_outlined), label: const Text('Extracurricular')),
          ]),
        )),
      );
      if (!mounted || choice == null) return;
      if (choice == AlumnoBusquedaScope.curricular) await _abrirCurricular(inst); else await _abrirExtracurricular(inst);
      return;
    }
  ...
  Future<void> _abrirCurricular(Institucion inst) async {
    await Navigator.push(context, MaterialPageRoute(builder: (_) => AlumnoVacantesCurricularesPage(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: ListView(
        ...
        children: [
          Text('¿Qué estás buscando?', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          const Text('Elegí primero el tipo de propuesta. Después podrás aplicar los filtros específicos correspondientes.'),
          const SizedBox(height: 14),
          _scopeCard(scope: AlumnoBusquedaScope.curricular, icon: Icons.school_outlined),
          _scopeCard(scope: AlumnoBusquedaScope.extracurricular, icon: Icons.category_outlined),
          if (_scope != null) ...[
            ...
            if (_cargando)
              ...
              Center(child: Padding(
                ...
                child: Text('No se encontraron instituciones con los criterios seleccionados.', textAlign: TextAlign.center),
              ))
            else ...[
              Text('Instituciones encontradas: ${_resultados.length}', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900)),
              const SizedBox(height: 10),
              for (final result in _resultados) ...[_buildResultCard(result), const SizedBox(height: 10)],
            ],
          ],
        ],
      )),
